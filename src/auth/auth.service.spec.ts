...
describe('AuthService', () => {
  ...
  it('generateJwtToken should call JwtService with payload and options', async () => {
    ...
    const payload = { email: 'user@example.com', username: 'user', displayName: null, isVerified: false };

    const token = await service.generateJwtToken(payload, { expiresIn: '1h' });

    expect(jwtService.signAsync).toHaveBeenCalledWith(payload, { expiresIn: '1h' });
  ...
  it('generateJwtToken should propagate JwtService errors', async () => {
    ...
    await expect(service.generateJwtToken(payload)).rejects.toThrow('jwt-failed');
  });
});
