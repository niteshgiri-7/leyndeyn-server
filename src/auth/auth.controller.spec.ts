...
describe('AuthController', () => {
  ...
  describe('login', () => {
    it('throws NotFoundException when user is missing', async () => {
      ...
      await expect(
        controller.login({ email: 'missing@example.com', password: 'pass1234' }),
      ).rejects.toBeInstanceOf(NotFoundException);
    });

    it('throws NotFoundException when user has no email or password hash', async () => {
      userRepository.findByEmail.mockResolvedValue({ id: '1', email: null, passwordHash: null });
    ...
    it('returns tokens for valid credentials', async () => {
      ...
      authService.generateJwtToken.mockResolvedValueOnce('access').mockResolvedValueOnce('refresh');

      const result = await controller.login({ email: 'user@example.com', password: 'pass1234' });
    ...
    it('propagates errors from AuthService.comparePassword', async () => {
      ...
      authService.comparePassword.mockRejectedValue(new Error('compare-failed'));
  ...
  describe('signup', () => {
    it('throws ConflictException when user already exists', async () => {
      userRepository.findByEmail.mockResolvedValue({ id: '1', email: 'user@example.com' });

      await expect(
        controller.signup({ email: 'user@example.com', username: 'user', password: 'pass1234' }),
