...
describe('UserService', () => {
  ...
  it('findUserById throws when user missing', async () => {
    ...
    await expect(service.findUserById('id-1')).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('findUserByEmail throws when user missing', async () => {
    ...
    await expect(service.findUserByEmail('user@example.com')).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('findUserByEmail returns user when found', async () => {
    ...
    await expect(service.findUserByEmail('user@example.com')).resolves.toEqual(user);
  });

  it('findAllUsers throws when no users', async () => {
    userRepository.findAll.mockResolvedValue([]);

    await expect(service.findAllUsers()).rejects.toBeInstanceOf(NotFoundException);
  });
  ...
  it('createUser propagates repository errors', async () => {
    ...
    await expect(service.createUser({ email: 'user@example.com' })).rejects.toThrow('create-failed');
  });
});
